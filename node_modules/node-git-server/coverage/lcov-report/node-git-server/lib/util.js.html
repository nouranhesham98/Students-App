...
<head>
    <title>Code coverage report for node-git-server/lib/util.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">node-git-server/lib</a> util.js</h1>
        <div class='clearfix'>
...
const Util = {
  ...
  basicAuth: function basicAuth(req, res, callback) {
      if(!req.headers["authorization"]) {
          ...
          <span class="missing-if-branch" title="else path not taken" >E</span>if (tokens[0] === "Basic") {
...
<span class="cstat-no" title="statement not covered" >          cmd = ['git', service, '--stateless-rpc', '--advertise-refs', repoLocation];</span>
      } else {
          cmd = ['git-' + service, '--stateless-rpc', '--advertise-refs', repoLocation];
      }
      
      const ps = spawn(cmd[0], cmd.slice(1));
      ps.on('error', <span class="fstat-no" title="function not covered" >(e</span>rr) =&gt; {
...
&nbsp;
    dup.once('reject', <span class="fstat-no" title="function not covered" >(c</span>ode) =&gt; {
...
&nbsp;
            git.emit('info', dup);
            <span class="missing-if-branch" title="else path not taken" >E</span>if (!anyListeners) dup.accept();
        } else <span class="missing-if-branch" title="if path not taken" >I</span>if (!ex) {
